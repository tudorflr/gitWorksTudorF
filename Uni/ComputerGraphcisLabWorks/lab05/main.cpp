...
void processEvents() {
    ...
    while (SDL_PollEvent(&currentEvent)) {
        ...
        if(currentEvent.type == SDL_EVENT_KEY_DOWN) {
            ...
            case SDLK_LEFT:
                p.rotate(10.0f);  // Rotate polygon by 10 degrees clockwise
                break;

            case SDLK_RIGHT:
                p.rotate(-10.0f); // Rotate polygon by -10 degrees (counterclockwise)
                break;

            case SDLK_R:
                p.transformationMatrix = mat3(); // Reset transformations
                break;

            case SDLK_UP:
                p.scale(1.1f);  // Scale up by 10%
                break;

            case SDLK_DOWN:
                p.scale(0.9f);  // Scale down by 10%
                break;
